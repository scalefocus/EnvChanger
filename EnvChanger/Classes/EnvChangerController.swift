...
public class EnvChangerController<T>: UIViewController where T: EnvironmentRepresentable {
    ...
    private let ACTIVE_ENV_KEY = "ENV_CHANGER_CURRENT_SAVED_ENVIRONMENT"
    private let window = EnvChangerWindow()
    private(set) var button: UIButton!
    private var buttonConfiguration: EnvButtonConfiguration!
    ...
    public func getSavedEnvironment() -> String {
    ...
    private func setupUI() {
        ...
        button.frame = CGRect(origin: CGPoint(x: view.frame.minX + 60, y: view.frame.minY + 60), size: button.bounds.size)
    ...
    private func actionHandler(sender: UIAlertAction) {
        ...
        let alert = UIAlertController(title: "Environment successfully changed.", message: "Your active environment is now: \n \(selectedEnv.environmentTitle)", preferredStyle: .alert)
